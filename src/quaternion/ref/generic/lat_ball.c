...
quat_lattice_bound_parallelogram_given_dualG(ibz_vec_4_t *box, ibz_mat_4x4_t *U, quat_lattice_t *dualG, const ibz_t *radius){
    ...
    quat_lll_core(&dualG->basis, U);

    // Compute the parallelogram's bounds
    int trivial = 1;
    for (int i = 0; i < 4; i++) {
        ibz_mul(&(*box)[i], &(dualG->basis)[i][i], radius);
        ibz_div(&(*box)[i], &rem, &(*box)[i], &dualG->denom);
...
quat_lattice_sample_from_ball_given_dualG(quat_alg_elem_t *res,
    ...
    ibz_mul(&rad, radius, &ibz_const_two);

    // Compute a bounding parallelogram for the ball, stop if it only
    // contains the origin
    int ok = quat_lattice_bound_parallelogram_given_dualG(&box, &U, dualG, &rad);
